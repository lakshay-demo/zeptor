      ...
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-4 lg:flex">
          ...
          {isLoggedIn ? (
            ...
            <>
              <Link to="/login" className="text-xs uppercase tracking-[0.2em] text-silver/80 hover:text-white">Login</Link>
              <Link to="/signup" className="btn-primary px-5 py-2 text-[10px]">Sign Up</Link>
            </>
          )}
          ...
          >
            <div className="absolute right-4 top-4 w-[calc(100%-2rem)] max-w-sm rounded-3xl border border-violet/20 bg-[#101017] p-6 shadow-[0_20px_60px_rgba(168,85,247,0.15)]" onClick={(event) => event.stopPropagation()}>
              ...
              {isLoggedIn ? (
                ...
                <div className="mt-6 space-y-3">
                  <Link to="/login" onClick={() => setIsOpen(false)} className="btn-secondary w-full px-5 py-3 text-sm">LOGIN</Link>
                  <Link to="/signup" onClick={() => setIsOpen(false)} className="btn-primary w-full px-5 py-3 text-sm">SIGN UP</Link>
                </div>
              )}
