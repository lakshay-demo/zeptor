  ...
  return (
    <div className="mx-auto max-w-7xl px-4 pb-16 pt-10 sm:px-6 lg:px-8">
      <section className="rounded-[40px] border border-white/10 bg-[#0b0b13] p-8 shadow-card">
        <p className="text-sm uppercase tracking-[0.35em] text-violet/70">Leaderboard</p>
      ...
      <section className="mt-10 grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="space-y-6">
          <div className="rounded-[36px] border border-white/10 bg-[#0f0f18] p-6 shadow-card">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-sm uppercase tracking-[0.35em] text-violet/70">Search team</p>
              ...
              <div className="relative w-full sm:w-80">
                ...
                <input
                  ...
                  className="w-full rounded-3xl border border-white/10 bg-[#0b0b13] py-3 pl-11 pr-4 text-sm text-white outline-none transition focus:border-violet focus:ring-2 focus:ring-violet/20"
                />
              </div>
            </div>
          </div>
          ...
          <div className="rounded-[36px] border border-white/10 bg-[#0f0f18] p-6 shadow-card">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-sm uppercase tracking-[0.35em] text-violet/70">Leaderboard table</p>
            ...
            <div className="mt-6 overflow-x-auto">
              <table className="min-w-full text-left text-sm text-silver/80">
                <thead>
                  <tr className="border-b border-white/10 text-xs uppercase tracking-[0.35em] text-violet/70">
                ...
                <tbody>
                  {filteredTeams.map((team) => (
                    <tr key={team.id} className={`border-b border-white/10 transition ${team.status === 'qualified' ? 'bg-white/5' : 'hover:bg-white/5'}`}>
              ...
              {filteredTeams.length === 0 && (
                <div className="rounded-3xl bg-white/5 p-8 text-center text-sm text-silver/80">No teams found. Try another search term.</div>
              )}
            </div>
          </div>
        </div>
        ...
        <aside className="space-y-6">
          <div className="rounded-[36px] border border-white/10 bg-[#0f0f18] p-6 shadow-card">
            <p className="text-sm uppercase tracking-[0.35em] text-violet/70">Top podium</p>
            <div className="mt-6 space-y-4">
              {leaderboardTeams.slice(0, 3).map((team) => (
                <motion.div key={team.id} className="rounded-3xl border border-white/10 bg-[#12121b] p-4">
