...
const HomePage = () => {
  ...
  return (
    <div className="home-shell">
      ...
      <section className="hero-content mx-auto max-w-7xl px-4 pb-20 pt-12 sm:px-6 lg:px-8 lg:pt-20">
            ...
            <Reveal key={session.id} delay={index * 0.05}>
              <article className="scrim-card">
                ...
                <div className="mt-7 grid gap-2 sm:grid-cols-3">
                  {session.entryOptions.map((option, optionIndex) => <div key={option.entryFee} className={`entry-tile ${entryColors[optionIndex]}`}><p className="entry-fee">₹{option.entryFee}</p><p className="entry-copy">₹{option.prizePool} prize pool</p><Link to={`/scrims/${session.id}`} className="entry-link"><span className="entry-link-label">BOOK NOW</span></Link></div>)}
                </div>
              </article>
            </Reveal>
          ))}
